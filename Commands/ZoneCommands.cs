...
namespace Lockpoint.Commands
{
    public class ZoneCommands
    {
        ...
        public void OnCommandAddZone(CCSPlayerController? player, CommandInfo commandInfo)
        {
            ...
            {
                commandInfo.ReplyToCommand("You already have an active zone. Use /endzone to finish it first.");
                return;
            }
            ...
            commandInfo.ReplyToCommand($"Zone '{zoneName}' started. Use /addpoint to add more points, /endzone to finish.");
        }
        ...
        public void OnCommandAddPoint(CCSPlayerController? player, CommandInfo commandInfo)
        {
            ...
            if (!_activeZones.ContainsKey(player))
            {
                commandInfo.ReplyToCommand("You don't have an active zone. Use /addzone [name] to start one.");
                return;
            }
        ...
        public List<Zone> GetCompletedZones() => _completedZones;
    }
}